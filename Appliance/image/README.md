...
It verifies the image checksum before device selection, lists only external physical whole disks plus positively identified removable cards in Apple's built-in SD reader, shows each model/capacity/protocol/mounted volume, rejects the current system disk and other internal/virtual/read-only media, and requires two exact confirmations. The warning explicitly states that every partition and file on the selected card will be destroyed. It checks fresh device membership plus the card's media UUID or built-in-reader card serial and physical-location identity after confirmation, after administrator authentication, and after unmounting. A narrow root worker reopens and verifies the image against the original pre-selection checksum, opens and pins the raw device descriptor, repeats all destination and identity checks, writes, fsyncs, seeks to byte zero, and verifies the complete written-image SHA-256 through that same descriptor before macOS can auto-mount and modify the boot filesystem. It then rechecks identity before ejecting. If a reader/card exposes no reliable media and physical-location identity, the script fails closed; use Raspberry Pi Imager for that hardware.
