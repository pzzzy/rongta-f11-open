...
install -m0755 "$ROOT/image/personalize-card.py" "$DIST/personalize-card.py"
install -m0644 "$ROOT/image/settings.example.toml" "$DIST/settings.example.toml"
...
(cd "$DIST" && sha256 "$product-$version.img.xz" "$product-$version.img.xz.sha256" "$product-$version.bmap" "$product-$version.manifest.json" "$product-$version.base-image.lock" "$product-$version.packages.tsv" "$product-$version-source.tar.gz" README.md LICENSE flash-card.py personalize-card.py settings.example.toml >SHA256SUMS)
