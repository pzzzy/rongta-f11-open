...
<div class="step"><h2>6. No-paper renderer validation</h2>{{if eq .ErrorStage "preview"}}<div class="error" data-error-stage="preview" role="alert">{{.ErrorText}}</div>{{end}}<p>Validate banner, gift, and raid rendering without sending paper. Generated files stay private to the appliance and are not presented as user-reviewable images.</p>{{if .State.Completed "preview"}}<span class="done">Renderer validation complete</span>{{else}}<form method="post" action="/action/preview"><input type="hidden" name="csrf_token" value="{{.CSRF}}"><button>Validate renderers without paper</button></form>{{end}}</div>
